...
class LandingView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Get certificates and trust badges
        from .models import Certificate
        context['certificates'] = Certificate.objects.filter(is_active=True, certificate_type='certificate')
        context['trust_badges'] = Certificate.objects.filter(is_active=True, certificate_type='trust_badge')
...
def create_booking_record(order, request):
    ...
    try:
        ...
        customer_dob = None
        dob_str = order.get('customer', {}).get('date_of_birth', '')
        if dob_str:
            try:
                from datetime import date
                customer_dob = datetime.strptime(dob_str, '%Y-%m-%d').date()
            except:
                pass
        ...
        booking = Booking.objects.create(
            ...
            customer_date_of_birth=customer_dob,
            customer_passport_number=order.get('customer', {}).get('passport_number', ''),
            emergency_contact_name=order.get('customer', {}).get('emergency_contact_name', ''),
            emergency_contact_phone=order.get('customer', {}).get('emergency_contact_phone', ''),
            special_instructions=order.get('customer', {}).get('special_instructions', ''),
            additional_notes=order.get('customer', {}).get('additional_notes', ''),
...
def send_booking_email(order, request, test_recipients=False):
    ...
    try:
        ...
        guest_state = customer.get('state') or ''
        guest_zip = customer.get('zip') or ''
        guest_country = customer.get('country') or ''
        guest_company = customer.get('company') or ''
        guest_dob = customer.get('date_of_birth') or ''
        guest_passport = customer.get('passport_number') or ''
        emergency_name = customer.get('emergency_contact_name') or ''
        emergency_phone = customer.get('emergency_contact_phone') or ''
        special_instructions = customer.get('special_instructions') or ''
        additional_notes = customer.get('additional_notes') or ''
            ...
            <div class="container">
                ...
                <div class="section">
                    ...
                    <table>
                        ...
                        {f"<tr><th>State/Province</th><td>{guest_state}</td></tr>" if guest_state else ''}
                        <tr><th>ZIP</th><td>{guest_zip}</td></tr>
                        {f"<tr><th>Country</th><td>{guest_country}</td></tr>" if guest_country else ''}
                        {f"<tr><th>Company</th><td>{guest_company}</td></tr>" if guest_company else ''}
                        {f"<tr><th>Date of Birth</th><td>{guest_dob}</td></tr>" if guest_dob else ''}
                        {f"<tr><th>Passport Number</th><td>{guest_passport}</td></tr>" if guest_passport else ''}
                        {f"<tr><th>Emergency Contact</th><td>{emergency_name} - {emergency_phone}</td></tr>" if emergency_name or emergency_phone else ''}
                        {f"<tr><th>Special Instructions</th><td>{special_instructions}</td></tr>" if special_instructions else ''}
                        {f"<tr><th>Additional Notes</th><td>{additional_notes}</td></tr>" if additional_notes else ''}
                    </table>
                </div>
                <div class="section">
                    <h3>Trip</h3>
        ...
        text_body = (
            ...
            f"Email: {guest_email}\n"
            f"Phone: {guest_phone}\n"
            + (f"Address: {guest_address}\n" if guest_address else "")
            + (f"City: {guest_city}\n" if guest_city else "")
            + (f"State/Province: {guest_state}\n" if guest_state else "")
            + (f"ZIP: {guest_zip}\n" if guest_zip else "")
            + (f"Country: {guest_country}\n" if guest_country else "")
            + (f"Company: {guest_company}\n" if guest_company else "")
            + (f"Date of Birth: {guest_dob}\n" if guest_dob else "")
            + (f"Passport Number: {guest_passport}\n" if guest_passport else "")
            + (f"Emergency Contact: {emergency_name} - {emergency_phone}\n" if emergency_name or emergency_phone else "")
