...
def bookings_list(request):
	qs = Booking.objects.select_related('pickup_location1', 'dropoff_location1', 'car_id', 'car_id__car_type').all().order_by('-date_capture')
...
def bookings_csv(request):
	...
	rows = []
	for b in qs:
		service = 'RT' if (b.trip_type == 'roundtrip' or not b.one_way) else 'OW'
		vehicle_name = b.car_id.name if b.car_id and b.car_id.name else (b.car_id.car_type.name if b.car_id and b.car_id.car_type else '')
		rows.append([
			b.id,
			b.pickup_location1.name if b.pickup_location1 else '',
			b.dropoff_location1.name if b.dropoff_location1 else '',
			b.how_people or 0,
			vehicle_name,
			b.pickup_date_time.strftime('%Y-%m-%d'),
			b.pickup_date_time.strftime('%H:%M'),
			b.flight_number or '',
			b.customer_first_name or '',
			b.customer_last_name or '',
			float(b.total_amount or 0),
			b.payment_method or '',
			service,
			b.client_id,
			b.customer_phone or '',
			b.return_date_time.strftime('%Y-%m-%d') if b.return_date_time else '',
			b.return_date_time.strftime('%H:%M') if b.return_date_time else '',
		])
	return _write_csv_response('bookings.csv', [
		'Order', 'Origin', 'Destination', 'Passenger', 'Vehicle', 'Date', 'Time', 
		'Flight', 'First Name', 'Last Name', 'Pay', 'Payment', 'Service', 
		'Email', 'Phone', 'Return Date', 'Return Time'
	], rows)
...
def sold_history_csv(request):
	...
	for b in Booking.objects.select_related('pickup_location1', 'dropoff_location1', 'car_id', 'car_id__car_type').all().order_by('-date_capture'):
		service = 'RT' if (b.trip_type == 'roundtrip' or not b.one_way) else 'OW'
		vehicle_name = b.car_id.name if b.car_id and b.car_id.name else (b.car_id.car_type.name if b.car_id and b.car_id.car_type else '')
		rows.append([
			b.id,
